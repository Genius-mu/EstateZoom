...
import { Menu, X, Home, Search, Heart, Phone, User, ChevronDown, MapPin } from 'lucide-react';
...
const Header = () => {
  ...
  const navLinks = [
    { name: 'Buy', hasDropdown: true },
    { name: 'Rent', hasDropdown: true },
    { name: 'Sell', hasDropdown: false },
    { name: 'Saved Homes', icon: <Heart size={18} />, hasDropdown: false },
    { name: 'Contact', icon: <Phone size={18} />, hasDropdown: false },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          >
            ...
            <div>
              <h1 className={`text-3xl font-extrabold tracking-tight ${isScrolled ? 'text-gray-900' : 'text-white'}`}>
                DreamHome
              </h1>
              <p className={`text-xs tracking-wider ${isScrolled ? 'text-gray-500' : 'text-white/70'}`}>
            ...
            {navLinks.map((link, index) => (
              ...
              >
                ...
                >
                  ...
                  <span>{link.name}</span>
                  {link.hasDropdown && <ChevronDown size={16} className="transition-transform group-hover:rotate-180" />}
                </a>
                ...
                {link.hasDropdown && (
                  <div className="absolute top-full left-0 mt-2 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                    <div className="bg-white rounded-2xl shadow-2xl py-3 border border-gray-100">
                      <a href="#" className="block px-6 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600">Houses</a>
                      <a href="#" className="block px-6 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600">Apartments</a>
                      <a href="#" className="block px-6 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600">Condos</a>
                    </div>
                  </div>
                )}
              </motion.div>
          ...
          <div className="hidden lg:flex items-center space-x-4">
            <motion.button
              className={`p-3 rounded-xl transition-all ${isScrolled ? 'hover:bg-gray-100' : 'hover:bg-white/20'}`}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
            >
              <Search size={24} className={isScrolled ? 'text-gray-700' : 'text-white'} />
            </motion.button>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
            >
              <User size={24} className={isScrolled ? 'text-gray-700' : 'text-white'} />
            </motion.button>
          ...
          >
            {isMobileMenuOpen ? (
              <X size={32} className={isScrolled ? 'text-gray-900' : 'text-white'} />
            ) : (
              <Menu size={32} className={isScrolled ? 'text-gray-900' : 'text-white'} />
            )}
          </button>
        </div>
      </div>
